...
async def get_roles(current_user: schemas.User = Depends(deps.get_current_user)):
    ...
    try:
        with conn.cursor() as cur:
            ...
            logger.info(f"Fetching roles for user_id: {current_user.id}")
            cur.execute("""
            ...
            rows = cur.fetchall()
            logger.info(f"Found {len(rows)} roles for user_id: {current_user.id}")
            for row in rows:
                logger.info(f"  Role: {row[1]}, candidate_count: {row[2]}")
                roles_list.append({
                    "id": row[0],
                    "name": row[1],
...
async def create_role(role: schemas.RoleCreate, current_user: schemas.User = Depends(deps.get_current_user)):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                INSERT INTO recruitment_roles (user_id, name)
                VALUES (%s, %s) RETURNING id, name
            """, (current_user.id, role.name))
            result = cur.fetchone()
            conn.commit()
            
            return {"message": f"Role '{result[1]}' created", "id": result[0], "name": result[1]}
    finally:
        return_db_connection(conn)
